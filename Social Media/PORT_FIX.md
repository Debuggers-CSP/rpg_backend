...
**Your backend is running on:** `localhost:8304` ✅
...
**Change the port from `8585` to `8304`:**
...
// AFTER (Correct):
const javaURI = "http://localhost:8304";
```
...
export const javaURI = "http://localhost:8304";  // ← Make sure it's 8304!
...
Edit `assets/js/api/config.js` and change `8585` → `8304`
...
```bash
curl http://localhost:8304/api/post/all
# Should return: []
```
...
| Service | Port | Status |
|---------|------|--------|
| **Flask Backend** | **8304** | ✅ Correct - Use this! |
...
1. **Backend is on 8304** - This is confirmed and working ✅
2. **Don't change backend** - It's correct!
3. **Change frontend config** - Update to port 8304
...
Once you update the port to `8304`:
...
```javascript
// Backend API Configuration
export const javaURI = "http://localhost:8304";
...
# Find and replace 8585 with 8304
find assets -name "*.js" -type f -exec sed -i '' 's/8585/8304/g' {} +
...
| What | Where | Change |
|------|-------|--------|
| **Backend** | Flask (running) | Port **8304** ✅ |
| **Frontend Config** | `config.js` | Change to **8304** ❌→✅ |
...
Just update that one line in your frontend config from `8585` to `8304` and everything will work!
